...
export class ServiceApi {
    ...
    private __fill_entityManager(data: any[]) {
        ...
        data.forEach(e => {
            ...
            this.entityManager.createEntity(this.get_masterEntityName(), e, breeze.EntityState.Unchanged, breeze.MergeStrategy.OverwriteChanges);
        });
    }
    ...
    exec_sql(sql: string): Q.Promise<any[]> {
    ...
    exec_sql_incr(input: sql_incr_input): Q.Promise<any> {
        ...
        this.exec_sql(sql_count).then((list: any[]) => {
            ...
            this.exec_sql(_sql).then((list2: any[]) => {
